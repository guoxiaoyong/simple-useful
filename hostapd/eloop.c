...
#define eloop_trace_sock_add_ref(table)                                        \
  do {                                                                         \
  } while (0)
#define eloop_trace_sock_remove_ref(table)                                     \
  do {                                                                         \
  } while (0)
...
static void eloop_sock_table_destroy(struct eloop_sock_table *table) {
  if (table) {
    ...
    for (i = 0; i < table->count && table->table; i++) {
      ...
      // wpa_trace_dump_funcname("eloop unregistered socket " "handler",
  ...
  dl_list_for_each(tmp, &eloop.timeout, struct eloop_timeout, list) {
    if (tmp->handler == handler && tmp->eloop_data == eloop_data &&
      ...
      if (os_reltime_before(&requested, &remaining)) {
        ...
        eloop_register_timeout(requested.sec, requested.usec, handler,
...
void eloop_run(void) {
  ...
  while (!eloop.terminate &&
    ...
    if (res < 0 && errno != EINTR && errno != 0) {
      wpa_printf(MSG_ERROR, "eloop: %s: %s", "select", strerror(errno));
      goto out;
    }
...
int eloop_terminated(void) { return eloop.terminate; }
