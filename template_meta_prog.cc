...
template <int X> struct MCD<X, 0> { static const long result = X; };
...
template <typename X, typename Y> struct Sum {
  ...
  static const long Den = B::Y::Den;
  typedef typename Simplify<Frak<Num, Den>>::result result;
};
...
template <> struct Factorial<0> { static const long result = 1; };
...
template <> struct E<0> { typedef Frak<1, 1> result; };
