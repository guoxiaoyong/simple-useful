...
int main() {
  ...
  if (setjmp(exception_env)) {
    ...
    printf("%s", strcpy((char *)mem_buffer,
                        "first succeeded!")); /* ... this will not happen */
  }
...
void first(void) {
  ...
  default: /* fall through */
    memcpy(exception_env, my_env,
           sizeof(jmp_buf)); /* restore exception stack */
    longjmp(exception_env,
            exception_type); /* continue handling the exception */
...
void second(void) {
  ...
  exception_type = 3;
  longjmp(exception_env,
          exception_type);    /* declare that the program has failed */
